...
- **Always update specs when implementing features** — when adding new features, endpoints, pages, or changing existing behavior, update the relevant spec files in `specs/` to match reality. Specs must reflect what is actually built, not just what was planned. If a spec doesn't exist for a new feature, create one. If a feature deviates from its spec, update the spec.
